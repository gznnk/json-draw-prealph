...
import { TwoColumnSplitView } from "./components/SplitView";
		...
		onSendMessage: async (message: string) => {
			...
			if (llmClient) {
				try {
					...
					setMessages((prevMessages) => [...prevMessages, assistantMessage]); // LLMClientを使用してレスポンスをストリーミング
		...
		<div className="App">
			<Page>
				<TwoColumnSplitView
					initialRatio={0.7}
					left={
						// Canvas view for the left pane
						<CanvasView content={defaultCanvasData} />
					}
					right={
						// Chat UI for the right pane
						<ChatUI {...chatConfig} />
					}
				/>
			</Page>
