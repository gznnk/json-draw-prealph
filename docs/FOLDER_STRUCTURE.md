...
| 現在の場所              | 推奨移動先           | 理由                                       |
|------------------------|----------------------|--------------------------------------------|
| `features/llm-client`  | `shared/llm-client`  | 複数の機能で使用、ユースケースに依存しない |
| `features/markdown`    | `shared/markdown`    | Markdownは機能横断で使われる              |
| `app/domain`           | `app/models`         | ビジネスロジックがない純粋な型定義である  |
